...
@pytest.mark.asyncio
async def test_bulk_insert_and_pagination(client: AsyncClient, superuser_token_headers: dict[str, str]):
    ...
    for role in roles_data:
        resp = await client.post(f"{settings.API_V1_STR}/roles", json=role,  headers=superuser_token_headers)
        assert resp.status_code == 201

    # test pagination → ambil 5 pertama
    resp = await client.get(f"{settings.API_V1_STR}/roles?page=1&limit=5&order_by=name&order_dir=asc", headers=superuser_token_headers)
    assert resp.status_code == 200
    data = resp.json()
    assert len(data["data"]) == 5

    # test pagination → ambil page 2
    resp = await client.get(f"{settings.API_V1_STR}/roles?page=2&limit=5&order_by=name&order_dir=asc", headers=superuser_token_headers)
    assert resp.status_code == 200
    data = resp.json()
    assert len(data["data"]) == 5
...
@pytest.mark.asyncio
async def test_filter_with_faker(client: AsyncClient, superuser_token_headers: dict[str, str]):
    ...
    # test filter by name
    resp = await client.get(f"{settings.API_V1_STR}/roles?name={unique_name}", headers=superuser_token_headers)
    assert resp.status_code == 200
    data = resp.json()
    ...
    resp = await client.get(f"{settings.API_V1_STR}/roles?search={keyword}", headers=superuser_token_headers)
    assert resp.status_code == 200
    data = resp.json()
